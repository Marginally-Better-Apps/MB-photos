...
The workflow can also be run manually against an existing `receiver-v*` tag. The tagged commit must be on `main` and have a successful `CI` run. The release job rebuilds and reruns the Windows integration suite before signing. The GitHub Release contains the signed portable `win-x64` ZIP and a SHA-256 checksum file. Published tags are immutable: the workflow refuses to replace assets on an existing release.
